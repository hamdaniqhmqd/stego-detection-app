...
export default function ProfilePage() {
    ...
    const uploadPhoto = async (file: File): Promise<string | null> => {
        ...
        if (uploadError) { setSaveError('Gagal upload foto: ' + uploadError.message); return null; }
        const { data } = supabaseAnonKey.storage.from('stego-images').getPublicUrl(filePath);
        if (!data?.publicUrl) { setSaveError('Gagal mendapatkan URL foto.'); return null; }
        if (!data.publicUrl.startsWith('https://')) { setSaveError('URL foto tidak valid: ' + data.publicUrl); return null; }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (fullnameChanged) updatePayload.fullname = fullname;
            if (photoChanged && photoFile) {
                const photoUrl = await uploadPhoto(photoFile);
                if (!photoUrl) { setIsSaving(false); return; }
    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            ...
            const { error: deleteErr } = await supabaseAnonKey
                .from('users')
    ...
    return (
        <DashboardLayoutAdmins>
            <section className="flex items-center justify-center lg:py-12 md:py-12 py-10">
                <div className="w-full max-w-2xl bg-neutral-100 rounded-md
                    ...
                    <div className="space-y-4 mb-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <div className="flex items-center gap-1.5 mb-2">
                                    <label className="block text-xs font-semibold text-neutral-800 tracking-wide">
                                        Username
                                    </label>
                                    <Tooltip text="Username bersifat unik dan tidak dapat diubah. Username digunakan untuk login ke sistem.">
                            ...
                            <div>
                                <div className="flex items-center gap-1.5 mb-2">
                                    ...
                                    <Tooltip text="Email terdaftar tidak dapat diubah. Hubungi administrator jika perlu mengganti email.">
                                        <span className="text-neutral-400 cursor-default">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="currentColor" viewBox="0 0 256 256">
                                                <path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm16-40a8,8,0,0,1-8,8,16,16,0,0,1-16-16V128a8,8,0,0,1,0-16,16,16,0,0,1,16,16v40A8,8,0,0,1,144,176ZM112,84a12,12,0,1,1,12,12A12,12,0,0,1,112,84Z" />
                                            </svg>
                        ...
                        <div>
                            <div className="flex items-center gap-1.5 mb-2">
                                ...
                                <Tooltip text="Nama lengkap yang akan ditampilkan di profil dan riwayat analisis. Bisa diubah kapan saja.">
                                    <span className="text-neutral-400 cursor-default">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="currentColor" viewBox="0 0 256 256">
                                            <path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm16-40a8,8,0,0,1-8,8,16,16,0,0,1-16-16V128a8,8,0,0,1,0-16,16,16,0,0,1,16,16v40A8,8,0,0,1,144,176ZM112,84a12,12,0,1,1,12,12A12,12,0,0,1,112,84Z" />
                                        </svg>
                        ...
                        <div>
                            <div className="flex items-center gap-1.5 mb-2">
                                <label className="block text-xs font-semibold text-neutral-900 tracking-wide">
                                    Role
                                </label>
                                <Tooltip text="Role menentukan hak akses kamu dalam sistem. Role hanya dapat diubah oleh administrator.">
                                    <span className="text-neutral-400 cursor-default">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="currentColor" viewBox="0 0 256 256">
                                            <path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm16-40a8,8,0,0,1-8,8,16,16,0,0,1-16-16V128a8,8,0,0,1,0-16,16,16,0,0,1,16,16v40A8,8,0,0,1,144,176ZM112,84a12,12,0,1,1,12,12A12,12,0,0,1,112,84Z" />
                                        </svg>
                            ...
                            <input
                                className="w-full px-3 py-3 rounded-md border border-neutral-600 bg-neutral-200 text-sm text-neutral-900 capitalize cursor-not-allowed"
                                value={user?.role ?? ''}
                                disabled
                                readOnly
                            />
                    ...
                    <div className="flex flex-col-reverse justify-between gap-4 md:flex-row md:gap-3">
                        ...
                        <Tooltip text="Menghapus akun secara permanen. Semua data analisis yang terkait juga akan diarsipkan. Tindakan ini tidak dapat dibatalkan.">
                            <button
                                className="flex items-center gap-2 px-6 py-3 rounded-sm
                                    text-red-950 font-semibold text-sm bg-red-100
                        ...
                        <Tooltip text={
                            isSaving
                                ...
                                : 'Simpan perubahan nama lengkap dan/atau foto profil ke database.'
                        }>
                            <button
                                className="flex items-center gap-2 px-6 py-3 rounded-sm
                ...
                >
                    <div className="bg-neutral-100 rounded-md w-full max-w-lg
                        ...
                        <div className="w-16 h-16 rounded-full bg-red-200 flex items-center justify-center mx-auto mb-4">
                            <svg className="w-8 h-8 text-red-800" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
                        ...
                        <div className="mb-1">
                            <div className="flex items-center gap-1.5 mb-1">
                                <label className="block text-xs font-medium text-neutral-900 uppercase tracking-wide">
                                    Password
                                </label>
                                <Tooltip text="Masukkan password akun kamu saat ini sebagai konfirmasi bahwa kamu adalah pemilik akun ini.">
                                    <span className="text-neutral-400 cursor-default">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="currentColor" viewBox="0 0 256 256">
                                            <path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm16-40a8,8,0,0,1-8,8,16,16,0,0,1-16-16V128a8,8,0,0,1,0-16,16,16,0,0,1,16,16v40A8,8,0,0,1,144,176ZM112,84a12,12,0,1,1,12,12A12,12,0,0,1,112,84Z" />
                                        </svg>
                            ...
                            <div className="relative">
                                <input
                                    className="w-full px-4 py-3 pr-11 rounded-md border
                                        border-neutral-600 text-sm outline-none text-neutral-900
                                    ...
                                    autoFocus
                                />
                                <Tooltip text={showPassword ? 'Sembunyikan password' : 'Tampilkan password'}>
                                    <button
                        ...
                        <div className="flex items-center gap-4 mt-8">
                            <Tooltip text="Tutup dialog ini tanpa menghapus akun.">
                                <button
                            ...
                            <Tooltip text={
                                !deletePassword
                                    ? 'Masukkan password terlebih dahulu untuk mengonfirmasi penghapusan akun.'
                                    : isDeleting
                                        ? 'Sedang memproses penghapusan akun...'
                                        : 'Konfirmasi dan hapus akun secara permanen. Data tidak dapat dipulihkan.'
                            }>
                                <button
                                    className="flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-sm
                                        text-red-950 font-semibold text-sm bg-red-100
