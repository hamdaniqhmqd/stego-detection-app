...
// components/Dashboard/sections/SectionPengguna.tsx
...
export function SectionPengguna({
    items,
    isLoading,
    hasMore,
    isLoadingMore,
    ...
    const stats = useMemo(() => ({
        ...
        verified: items.filter(i => i.is_verified && !i.deleted_at).length,
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <MiniStat label="Belum Terverifikasi" value={stats.active - stats.verified} color="text-amber-600" bg="bg-amber-50" border="border-amber-200" />
                <MiniStat label="Dihapus" value={stats.deleted} color="text-red-500" bg="bg-red-50" border="border-red-200" />
            </div>

            <TableShell
                title="Data Pengguna"
                subtitle="Kelola akun pengguna terdaftar"
                ...
                {isLoading ? (
                    ...
                    <tr
                        ...
                        className={`hover:bg-neutral-50/60 transition-colors
                            ...
                            ${pending === u.id ? 'pointer-events-none opacity-40' : ''}`}
                    >
                        {/* Pengguna */}
                        <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full">
                                    <img src={u.photo ?? `https://ui-avatars.com/api/?name=${u.fullname ?? u.username}&background=random&color=fff`} alt="Avatar" className="w-full h-full rounded-full object-cover" />
                                </div>
                                <div>
                                ...
                                {u.deleted_at ? (
                                    <>
                                        ...
                                        <ActionBtn
                                            ...
                                            danger
                                            onClick={() =>
                                                setConfirm({ type: 'hard', id: u.id, name: u.fullname ?? u.username })
                                            }
                                        />
                                    </>
                                ) : (
                                    <ActionBtn
                                        ...
                                        label="Nonaktifkan"
                                        onClick={() =>
                                            setConfirm({ type: 'soft', id: u.id, name: u.fullname ?? u.username })
                                        }
                                    />
                                )}
                            </div>
