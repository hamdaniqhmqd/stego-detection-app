        ...
        <>
            <aside
                ...
                className={`hidden md:flex md:flex-col
                    ...
                    h-full transition-all duration-300 bg-gray-950 border-r border-gray-700`}
                aria-label="Sidebar"
            >
                {/* Header */}
                <div className="px-4 py-3 flex items-center justify-between border-b border-gray-700">
                    {!isDesktopSidebarOpen && (
                        <Link href="/dashboard" className="hidden md:flex items-center text-lg font-semibold text-white line-clamp-1 text-nowrap">
                            Deteksi Stego
                        </Link>
                    )}
                    ...
                    <button
                        ...
                        className={`hidden md:block p-1 bg-gray-800 hover:bg-gray-900 rounded-md text-white transition-transform duration-300
                            ${isDesktopSidebarOpen ? 'rotate-180 mx-auto' : ''}`}
                    >
                ...
                {/* Navigation Menu */}
                <div className={`${isDesktopSidebarOpen ? 'flex-1' : 'shrink-0'} min-h-0 overflow-y-auto scrollbar_y_custom px-4 py-3`}>
                    {!isDesktopSidebarOpen && (
                        <p className="text-gray-400 hidden md:block text-xs mb-2 font-normal line-clamp-1 text-nowrap">Menu</p>
                    )}
                ...
                {/* Riwayat Analisis */}
                {!isDesktopSidebarOpen && (
                    <div className={`flex-1 min-h-0 px-4 py-3 border-t border-gray-800`}>

                        {/* Header section riwayat — hanya tampil di desktop expanded */}
                        <p className="text-gray-400 text-xs mb-2 font-normal line-clamp-1 text-nowrap hidden md:block">
                            Riwayat Analisis
                        </p>

                        <div className="overflow-y-auto scrollbar_y_custom h-full">
                ...
                <div className="px-3 py-3 border-t border-gray-700">
                    ...
                    <div className={`hidden lg:flex items-center gap-3 ${isDesktopSidebarOpen ? 'flex-col' : 'justify-between'}`}>
                        {!isDesktopSidebarOpen && (
                            <Link href="/dashboard/profile" className="flex items-center gap-2 min-w-0 flex-1">
                                ...
                                <div className="flex flex-col min-w-0">
                                    <h3 className="font-semibold text-xs text-gray-200 truncate">{user?.username || ''}</h3>
                                    <h3 className="font-light text-[10px] text-gray-400 truncate">{user?.email || ''}</h3>
                                </div>
                            </Link>
                        )}
                        {isDesktopSidebarOpen && (
                        ...
                        {!isDesktopSidebarOpen && (
                            <button
                                ...
                                className="flex items-center justify-center p-2 rounded-lg hover:bg-gray-800 text-gray-300 hover:text-white transition-colors duration-200 group relative"
                                title="Logout"
                            >
                    ...
                    <div className={`lg:hidden flex items-center gap-2 ${isDesktopSidebarOpen ? 'flex-col' : 'justify-between'}`}>
                        <Link href="/dashboard/profile" className='flex items-center gap-2'>
                            ...
                            {!isDesktopSidebarOpen && (
                                <div className="flex flex-col min-w-0">
                                    <h3 className="font-semibold text-xs text-gray-200 line-clamp-1">{user?.username || ''}</h3>
                                    <h3 className="font-light text-[10px] text-gray-400 line-clamp-1">{user?.email || ''}</h3>
                                </div>
                            )}
                        </Link>
                        {!isDesktopSidebarOpen && (
                            <button
                                onClick={handleLogout}
                                className="p-1 rounded-lg hover:bg-gray-800 text-gray-300 hover:text-white transition-colors group relative"
                                title="Logout"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M16 13v-2H7V8l-5 4 5 4v-3z" />
            ...
            >
                <div className="flex flex-col h-full bg-gray-950 shadow-2xl border-r border-gray-700">
                    ...
                    {/* Mobile Nav */}
                    <div className="shrink-0 overflow-y-auto scrollbar_y_custom px-4 py-4">
                        <ul className="font-medium space-y-1">
                    ...
                    {/* Mobile Riwayat */}
                    <div className="flex-1 min-h-0 overflow-y-auto scrollbar_y_custom px-4 py-3 border-t border-gray-800">
                        <p className="text-gray-400 text-sm mb-2 font-normal">Riwayat Analisis</p>
                        <RiwayatAnalisisSidebar
                            userId={user?.id}
                            isCollapsed={false}
                        />
                    ...
                    <div className="px-4 py-4 border-t border-gray-700 flex items-center justify-between">
                        <Link href={'/dashboard/profile'} className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <h3 className="text-sm font-semibold text-white line-clamp-1">{user?.username || ''}</h3>
                                <p className="text-xs text-gray-400 line-clamp-1">{user?.email || ''}</p>
                            </div>
                        </Link>
                        <button
                            onClick={handleLogout}
                            className="flex items-center justify-center gap-2 p-3 rounded-lg bg-gray-900 text-white transition-colors"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M16 13v-2H7V8l-5 4 5 4v-3z" />
                                <path d="M20 3h-9c-1.103 0-2 .897-2 2v4h2V5h9v14h-9v-4H9v4c0 1.103.897 2 2 2h9c1.103 0 2-.897 2-2V5c0-1.103-.897-2-2-2z" />
